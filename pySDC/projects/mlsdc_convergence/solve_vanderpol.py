...
def solve_vanderpol(m, random_init, niter_arr, nsteps_arr, only_uend, fname_errors):
    """
    Run SDC and MLSDC for Van der Pol ODE with given parameters
...
#    controller_params['predict'] = False
    ...
    description_sdc['problem_class'] = vanderpol
...
#    description_sdc['dtype_f'] = mesh
    ...
    fin = open("data/lsg_vanderpol.pickle", "rb")
    lsg = pickle.load(fin)
    fin.close()
    ...
    for niter in niter_arr:
        ...
        for i, nsteps in enumerate(nsteps_arr):
            ...
            order_sdc = log(error_sdc[(niter, nsteps_arr[i-1])]/err_sdc) / \
                log(nsteps/nsteps_arr[i-1]) if i > 0 and error_sdc[(niter, nsteps_arr[i-1])] > 0 else 0
            ...
            order_mlsdc = log(error_mlsdc[(niter, nsteps_arr[i-1])]/err_mlsdc) / \
                log(nsteps/nsteps_arr[i-1]) if i > 0 and error_mlsdc[(niter, nsteps_arr[i-1])] > 0 else 0
            ...
            order_uend_sdc = log(error_uend_sdc[(niter, nsteps_arr[i-1])]/err_uend_sdc) / \
                log(nsteps/nsteps_arr[i-1]) if i > 0 and error_uend_sdc[(niter, nsteps_arr[i-1])] > 0 else 0
            ...
            order_uend_mlsdc = log(error_uend_mlsdc[(niter, nsteps_arr[i-1])]/err_uend_mlsdc) / \
                log(nsteps/nsteps_arr[i-1]) if i > 0 and error_uend_mlsdc[(niter, nsteps_arr[i-1])] > 0 else 0
    ...
    for j, niter in enumerate(niter_arr[:-1]):
        ...
        for i, nsteps in enumerate(nsteps_arr):
            ...
            order = log(error_k_sdc[(niter, nsteps_arr[i-1])]/error_k_sdc[(niter, nsteps)]) / \
            ...
            order = log(error_k_mlsdc[(niter, nsteps_arr[i-1])]/error_k_mlsdc[(niter, nsteps)]) / \
                log(nsteps/nsteps_arr[i-1]) if i > 0 else 0
...
if __name__ == "__main__":
    ...
    if only_uend:
        ...
        figname = "figures/errors_vanderpol_uend.png"
    else:
