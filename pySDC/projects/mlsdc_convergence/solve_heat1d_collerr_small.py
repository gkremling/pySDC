    ...
    for i, err in enumerate(error):
        ...
        for j, niter in enumerate(err['niter_arr']):
            ...
            axes[i].plot(dt_arr, [err[(niter, n)] for n in err['nsteps_arr']], color=color[j],
                         marker=marker[j], markersize=10, linestyle='None', label='k={}'.format(niter))

            # erwartete Linie: err
            axes[i].plot(dt_arr, [(err[(niter, err['nsteps_arr'][0])] / \
                ((nstep/err['nsteps_arr'][0]) ** (order_sdc(niter)))) for nstep in err['nsteps_arr']], color=color[j])

        # erreichte Punkte
        axes[i].plot(dt_arr, [err[(n)] for n in err['nsteps_arr']], color=color[-1],
                     marker=marker[-1], markersize=10, linestyle='None', label='coll sol')

        # erwartete Linie: err
        axes[i].plot(dt_arr, [(err[(err['nsteps_arr'][0])]/((nstep/err['nsteps_arr'][0])**(order_coll(niter))))
                              for nstep in err['nsteps_arr']], color=color[-1])
