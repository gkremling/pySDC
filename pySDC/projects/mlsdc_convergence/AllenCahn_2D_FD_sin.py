...
class allencahn_sin_fullyimplicit(ptype):
    """
    ...
    def __init__(self, problem_params, dtype_u=mesh, dtype_f=mesh):
        ...
        essential_keys = ['nvars', 'nu', 'eps', 'newton_maxiter', 'newton_tol', 'lin_tol', 'lin_maxiter', 'freq']
        ...
        super(allencahn_sin_fullyimplicit, self).__init__(problem_params['nvars'], dtype_u, dtype_f, problem_params)
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        while n < self.params.newton_maxiter:
            ...
            g = u - factor * (self.A.dot(u) + eps2 * u * (1.0 - u ** nu)) - rhs.values.flatten()
            ...
            # assemble dg
            dg = Id - factor * (self.A + eps2 * sp.diags((1.0 - (nu + 1) * u ** nu), offsets=0))
    ...
    def eval_f(self, u, t):
        ...
        v = u.values.flatten()
        f.values = self.A.dot(v) + eps2 * v * (1.0 - v ** self.params.nu)
    ...
    def u_exact(self, t):
        ...
        me.values = np.array([np.array([np.sin(self.params.freq * np.pi * x) * np.sin(self.params.freq * np.pi * y) for x in self.xvalues]) for y in self.xvalues])
        return me
