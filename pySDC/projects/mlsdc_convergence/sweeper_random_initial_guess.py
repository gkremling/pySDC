...
#                xvalues = np.array([(i + 1) * P.dx for i in range(P.params.nvars)])
#                L.u[m].values = np.sin(np.pi * xvalues)
                L.f[m] = P.eval_f(L.u[m], L.time + L.dt * self.coll.nodes[m - 1])
            elif self.params.initial_guess == 'random':
                np.random.seed(m*30)
                tmp = P.dtype_u(P.init)
                if type(P.init) == tuple:
                    tmp.values = np.array([np.random.rand(P.init[0]) for i in range(P.init[1])])
                else:
